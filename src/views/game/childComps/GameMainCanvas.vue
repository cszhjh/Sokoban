...
export default {
  ...
  methods: {
    ...
    drawMap() {
      ...
      for (let i = 0; i < this.getCurrentMapLength; ++i) {
        for (let j = 0; j < this.getCurrentMapItemLength; ++j) {
          ...
          this.cxt.drawImage(
            img,
            WIDTH * j - (img.width - WIDTH) / 2,
            HEIGHT * i - (img.height - HEIGHT),
            img.width,
            img.height
          )
        }
    ...
    tryMove(roleTo, boxTo) {
      ...
      if (
        roleTo.x < 0 ||
        roleTo.x >= this.getCurrentMapLength ||
        roleTo.y < 0 ||
        roleTo.y >= this.getCurrentMapItemLength ||
        this.getCurrentMap[roleTo.x][roleTo.y] == 1
      ) {
        return false
      }
      ...
      if (this.getCurrentMap[roleTo.x][roleTo.y] == 3 || this.getCurrentMap[roleTo.x][roleTo.y] == 5) {
        if (
          boxTo.x < 0 ||
          boxTo.x >= this.getCurrentMapLength ||
          boxTo.y < 0 ||
          boxTo.y >= this.getCurrentMapItemLength ||
          this.getCurrentMap[boxTo.x][boxTo.y] == 1 ||
          this.getCurrentMap[boxTo.x][boxTo.y] == 3 ||
          this.getCurrentMap[boxTo.x][boxTo.y] == 5
        ) {
          return false
        }
      ...
      if (
        this.currentInitMap[this.rolePosition.x][this.rolePosition.y] == 2 ||
        this.currentInitMap[this.rolePosition.x][this.rolePosition.y] == 5
      ) {
        this.moveCurrentMap({
    ...
    checkPlayer() {
      for (let i = 0; i < this.currentInitMap.length; ++i)
        for (let j = 0; j < this.currentInitMap[i].length; ++j)
          if (
            (this.currentInitMap[i][j] == 2 && this.getCurrentMap[i][j] != 3) ||
            (this.currentInitMap[i][j] == 5 && this.getCurrentMap[i][j] != 3 && this.getCurrentMap[i][j] != 5)
          ) {
            return false
          }
      return true
